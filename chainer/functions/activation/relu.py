...
class ReLU(function_node.FunctionNode):
    ...
    def forward_ideep(self, inputs):
        ...
        cc = _ideep.ideep.xnn.ReLUForward(inputs)
...
def _heaviside(x):
    if _ideep.is_available() and isinstance(x, _ideep.ideep.mdarray):
...
class ReLUGradIdeep(ReLUGrad3Base):
    ...
    def forward(self, inputs):
        cc = _ideep.ideep.xnn.ReLUBackward((self.x,), inputs, self.hint)
        ggx, = cc.execute_on()
        return ggx,
